...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (status === 'processing' && fileId) {
      intervalId = setInterval(async () => {
        try {
          const res = await axios.get(`${API_BASE_URL}/api/status/${fileId}`);
  ...
  const fetchKeyInfo = async (id) => {
    try {
      const res = await axios.get(`${API_BASE_URL}/api/download/${id}/key.json`);
  ...
  const handleScaleSubmit = async () => {
    ...
    try {
      const res = await axios.post(`${API_BASE_URL}/api/scale`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
  ...
  const handleDownload = (fileName) => {
    window.location.href = `${API_BASE_URL}/api/download/${fileId}/${fileName}`;
  };
    ...
    <div className="results-container">
      ...
      {activeModule === 'separation' && ['vocals.wav', 'drums.wav', 'bass.wav', 'guitar.wav', 'piano.wav'].map((trackName) => (
        <div key={trackName} className="track-card">
          ...
          <audio controls src={`${API_BASE_URL}/api/download/${fileId}/${trackName}`} />
